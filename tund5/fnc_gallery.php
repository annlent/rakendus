<?php
	...
	function readAllSemiPublicPictureThumbs(){
		...
		while($stmt->fetch()){
			...
			// $html .= '<img src="../../uploadThumbnail/' .$filenameFromDb .'" alt="'.$altFromDb .'" class="thumb" data-fn="' .$filenameFromDb .'" data-id="' .$idFromDb .'">' ."\n \t \t";
	...
	function readAllMyPictureThumbsPage($page, $limit){
		...
		$html = "";
		$conn = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUserName"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		$stmt = $conn->prepare("SELECT id, filename, alttext FROM vr20_photos WHERE userid=? AND deleted IS NULL LIMIT ?,?");
		echo $conn->error;
	...
	function readAllPublicPictureThumbsPage($page, $limit){
		...
		while($stmt->fetch()){
			...
			// $html .= '<img src="../../uploadThumnail/' .$filenameFromDb .'" alt="'.$altFromDb .'" class="thumb" data-fn="' .$filenameFromDb .'" data-id="' .$idFromDb .'">' ."\n \t \t";
			$html .= "<p>" .$firstnameFromBb ." " .$lastnameFromDb ."</p> \n \t \t";
			$html .= "<p> Hinne: " .round($ratingFromDb, 2) ."</p> \n";
			$html .= "</div> \n \t \t";
		}
